...
public class KaffpaE extends Algorithm {
    private static final List<String> requiredParameters = Arrays.asList("processes", "k", "seed", "preconfiguration", "imbalance", "time_limit", "mh_enable_quickstart", "mh_optimize_communication_volume", "mh_enable_kabapE", "mh_enable_tabu_search", "kabaE_internal_bal", "balance_edges");
    ...
    public Object run(Structure s, AlgorithmParameters params, Set<Object> selection, ProgressHandler onprogress) throws Exception {
        ...
        String pValue = commandParameters.get(0).startsWith("--k") ? "2" : commandParameters.get(0);
        command.add("mpirun -n " + pValue + " " + workingDirectory + "./kaffpaE KaHIP_results/myGraph.graph ");
        command.add(String.join(" ", commandParameters.get(0).startsWith("--k") ? commandParameters : subCommand));
        command.add(" --output_filename=KaHIP_results/output");

        //Executing command
        String commandOutput = kaHIPUtil.executeCommand(String.join("", command));
